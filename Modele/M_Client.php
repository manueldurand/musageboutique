...
class M_Client
{
     ...
     * crée une entrée dans la table lafleur_villes sinon
     *
     * @param string $ville
     * @return int $idVille
     */
    ...
    {
        $conn = AccesDonnees::getpdo();
        $req = "SELECT id_ville FROM lafleur_villes WHERE ville = :v";
        ...
        if ($data != null) {
            ...
            $req3 = "INSERT INTO lafleur_villes(ville) VALUES (:v)";
    ...
    {
        $conn = AccesDonnees::getpdo();
        $req = "SELECT id_code_postal FROM lafleur_code_postal WHERE code_postal = :c";
        $stmt = $conn->prepare($req);
        $stmt->bindParam(':c', $cp);
        $stmt->execute();
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($data != null) {
            return $data['id_code_postal'];
        } else {
